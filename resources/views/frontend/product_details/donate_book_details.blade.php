<x-frontend.layout.master>
    ...
    <div class="container ">
        <div class="row">
            <div class="col-md-6 ">
                <img src="{{ asset('storage/Products/' . $bookdetail->bookimage) }}" alt="">
            </div>
            <div class="col-md-6 ">
                <div class="row">
                ...
                <div class="row">
                    ...
                    <h5><b>Book Quantity: </b>{{ $bookdetail->bookquantity }}</h5>
                    <h5><b>Book id: </b>{{ $bookdetail->id }}</h5>
                    <h5><b>Book Quantity: </b>{{ auth()->user()->id }}</h5>
                </div>
                <form class="form-horizontal" role="form" action="{{route('getdonates.store')}}" method="post" enctype="multipart/form-data">
                    <x-backend.alertmessage.alertmessage type="success"/>
                    @csrf
                    <x-frontend.form.input name="book_id" type="hidden" text="" value="{{$bookdetail->id }}"/>
                    <x-frontend.form.input name="user_id" type="hidden" text="" value="{{auth()->user()->id}}"/>
